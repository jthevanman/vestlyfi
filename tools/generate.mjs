...
function writeNeedsVerification(states) {
  ...
  lines.push('  - **Oregon — INDEXED (taxYearBasis 2025).** Now uses the federalTaxDeduction');
  lines.push('    cap-phaseout mode: 2025 cap $8,500, phased out $125k–$145k AGI (official OR-40 Table 4).');
  lines.push('  - **Missouri — deferred.** federalTaxDeduction percent-of-federal mode is ready (35/25/15/5/0%');
  lines.push('    by MO AGI, cap $5,000/$10,000, confirmed). Still pending: the exact 2025 MO graduated');
  lines.push('    bracket thresholds + top rate (4.7% believed) — the 4711 reference guide did not');
  lines.push('    contain an extractable rate table. Pull the MO-1040 rate schedule before indexing.');
  lines.push('  - **Wisconsin — deferred.** slidingStandardDeduction engine support is ready, but WI\'s');
  lines.push('    exact 2025/2026 standard-deduction formula (base, phase-out start, rate per filing');
  lines.push('    status) and bracket thresholds were not captured. Pull the WI Form 1 std-deduction table.');
